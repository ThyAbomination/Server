...
public class NPCDrops {
	...
	public void loadDrops() {
		try {
			int[][][] npcDrops = new int[NPCHandler.maxNPCs][][];
			int[][][] rareDrops2 = new int[NPCHandler.maxNPCs][][];
			int[] itemRarity = new int[NPCHandler.maxNPCs];
	...
	public void loadConstants() {
		try {
			File f = new File("./Data/cfg/NpcConstants.cfg");
			Scanner s = new Scanner(f);
			while (s.hasNextLine()) {
				String line = s.nextLine();
				if (line.startsWith("#"))
