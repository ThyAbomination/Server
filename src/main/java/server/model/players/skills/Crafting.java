...
public class Crafting {
	...
	public enum LeatherCrafting {
		...
		LEATHERVAMBS(1741, 1063, 11, 22, 1), LEATHERCHAPS(1741, 1095, 14, 27, 1), LEATHERBODY(
				1741, 1129, 18, 25, 1),

		GREENVAMBS(1745, 1065, 57, 62, 1), GREENCHAPS(1745, 1099, 60, 124, 2), GREENBODY(
				1745, 1135, 63, 186, 3),

		BLUEVAMBS(2505, 2487, 66, 70, 1), BLUECHAPS(2505, 2493, 68, 140, 2), BLUEBODY(
				2505, 2499, 71, 210, 3),

		REDVAMBS(2507, 2489, 73, 78, 1), REDCHAPS(2507, 2495, 75, 156, 2), REDBODY(
				2507, 2501, 77, 234, 3),

		BLACKVAMBS(2509, 2491, 79, 86, 1), BLACKCHAPS(2509, 2497, 82, 172, 2), BLACKBODY(
				2509, 2503, 84, 258, 3);
		...
		if (lea != null) {
			if (c.playerLevel[c.playerCrafting] >= lea.getReqLevel()) {
				if (c.getItems().playerHasItem(lea.getLeather(),
					...
					c.getPA().addSkillXP(
							lea.getXP() * Config.CRAFTING_EXPERIENCE,
							c.playerCrafting);
					resetCrafting();
				} else {
	...
	public enum GemCrafting {
		...
		OPAL(1625, 1609, 891, 1, 15, true), JADE(1627, 1611, 891, 13, 20, true), REDTOPAZ(
				...
				1603, 887, 34, 85, false), DIAMOND(1631, 1601, 890, 43, 108,
				false), DRAGONSTONE(6571, 1615, 890, 55, 138, false), ONYX(
				6571, 6585, 2717, 67, 168, false);
