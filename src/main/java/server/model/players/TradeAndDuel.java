...
public class TradeAndDuel {
	...
	public void requestTrade(int id) {
		try {
			...
			if (System.currentTimeMillis() - c.logoutDelay < 10000) {
				c.sendMessage("You cannot trade anyone while in combat!");
				return;
			}
			...
			if (!c.goodDistance(o.getX(), o.getY(), c.getX(), o.getY(), 1)
					|| !o.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(),
							1)) {
				c.sendMessage("Please move closer to the other player.");
				return;
			}
	...
	public void openTrade() {
		...
		c.canOffer = true;
		c.tradeStatus = 1;
		o.getPA().sendFrame126("" + Misc.formatPlayerName(c.playerName) + "",
				3451);
		c.getPA().sendFrame126("" + Misc.formatPlayerName(o.playerName) + "",
				3451);
	...
	public boolean tradeItem(int itemID, int fromSlot, int amount) {
		...
		for (int i : Config.ITEM_TRADEABLE) {
			if (i == itemID && !c.playerName.equalsIgnoreCase("Ultima")) {
	...
	public void giveItems() {
		...
		try {
			for (GameItem item : o.getTradeAndDuel().offeredItems) {
				...
				o.getTradeLog().tradeGive(c.getItems().getItemName(item.id),
						item.amount);
			}
	...
	public void requestDuel(int id) {
		try {
			...
			if (c.duelStatus == 0 && o.duelStatus == 0 && c.duelRequested
				...
				} else if (c.duelStatus >= 1 || o.duelStatus >= 1) {
					c.sendMessage("You or your opponent are already in a duel.");
				} else {
					c.sendMessage("Please move closer to the other player.");
				}
				...
				if (o.duelStatus == 0) {
					...
					o.sendMessage(Misc.optimizeText(c.playerName) + ":duelreq:");
				} else if (o.duelStatus >= 1) {
					c.sendMessage("This player is busy at the moment.");
				}
			}
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		...
		if (!Config.ADMIN_CAN_TRADE && c.playerRights >= 2
