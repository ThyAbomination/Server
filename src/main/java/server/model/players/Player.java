...
public abstract class Player {
	...
	public List<String> lastKilledIPs = new LinkedList<String>();

	public void addKill(Client killed) {
		if (killed != null) {
			if (lastKilledIPs.size() >= Config.KILLS_BEFORE_PKP_IP_GAIN) {
				lastKilledIPs.remove(0);
			}
			lastKilledIPs.add(killed.connectedFrom);
		} else {
			System.out.println("Killed is null");
		}
	}
	...
	public int NomadKills;
	public int AvatarKills;
	public int CursebearerKills;
	public int TormentedDemonKills;
	public int KalphiteQueenKills;
	public int BarrelChestKills;
	public int SaradominKills;
	public int ArmadylKills;
	public int BandosKills;
	public int ZamorakKills;
	public int PrimalKills;

	public boolean isUsingSpecial = true;
	public int overloadcounter = 0;
	public int totalDoses = 0;
	private boolean bankX = false;
	public int infraction = 0;
	public boolean onAuto = false;
	public String UUID = "";
	public int tzhaarToKill = 0, tzhaarKilled = 0, tzKekSpawn = 0,
			tzKekTimer = 0, caveWave, tzhaarNpcs;
	...
	public boolean defaultWealthTransfer, updateInventory, oldSpec,
			stopPlayerSkill, stopPlayerPacket;
	public boolean playerBFishing, finishedBarbarianTraining, ignoreDefence,
			secondFormAutocast, magicDef, spellSwap, hasBankPin, recoverysSet;
	public int rangeEndGFX, boltDamage, teleotherType, playerTradeWealth,
			doAmount, woodcuttingTree, stageT, playerDialogue;
	...
	public int[] questbkIds = { 8145, 8147, 8148, 8149, 8150, 8151, 8152,
	...
	public int processPackets = 0;
	public int destroyItem = 0;
	public int spiritWave = 0;
	public int spiritCount = 0;
	public boolean firstWave, canLoot = false;
	public boolean secondWave, thirdWave, fourthWave;
	public int barbsToKill = 0, barbsKilled = 0, barbWave = 0, foodCombo = 0,
			barbDamage = 0, barbLeader = 0;
	public int Streak;
	public int vlsLeft = 1500, statLeft = 1500, vTopLeft = 1500,
			vLegsLeft = 1500, sTopLeft = 1500, sLegsLeft = 1500,
			sHelmLeft = 1500, zHoodLeft = 1500, zStaffLeft = 1500,
			zTopLeft = 1500, zBottomLeft = 1500, mBodyLeft = 1500,
			mChapsLeft = 1500, mCoifLeft = 1500, vSpearLeft = 1500;
	...
	public int newItem;
	public int newXp;
	public boolean herbloreI;
	public int herbAmount;
	public boolean secondHerb;
	public boolean below459;
	public String getLine;
	public boolean inBarbDef = false;
	public boolean enterChallenge = false;
	public boolean enterCaves = false;
	...
	public int dfsCount;
	public boolean isFlaxSpinning;
	public boolean isFilling;
	public int pvpKills;
	public int pvpPoints, slayerPoints, barbPoints;
	...
	public boolean afterDuel = false;
	public double getstr, getatt, getdef, getran, getmag;
	...
	public int hideId;
	public int woodcutting[] = { 0, 0, 0, 1, -1, 2 };

	public int squares = 0;
	public boolean initialized = false, isCutting = false, vineWalk = false,
			...
			autocasting = false, usedSpecial = false, mageFollow = false,
			dbowSpec = false, craftingLeather = false, properLogout = false,
			secDbow = false, maxNextHit = false, ssSpec = false,
			vengOn = false, addStarter = false, accountFlagged = false,
			msbSpec = false, banking = false, isBanking = false;

	public int cwKills, cwDeaths, cwGames, saveDelay, playerKilled, pkPoints,
			...
			prayerLevelReq, rangeLevelReq, magicLevelReq, firemakingLevelReq,
			...
			pcPoints = 0, magePoints = 0, lastArrowUsed = -1, clanId = -1,
			autoRet = 0, pcDamage = 0, RFDToKill = 0, RFDKilled = 0, xInterfaceId = 0, xRemoveId = 0,
			xRemoveSlot = 0, waveId = 0, frozenBy = 0, poisonDamage = 0,
			bonusAttack = 0, lastNpcAttacked = 0, killCount = 0,
			altarPrayed = 0, actionTimer, height = 0, roundNpc = 0,
			addPlayerSize = 0, SolProtect = 0;
	...
	public long lastPlayerMove, lastoverload, lastPoison, lastPoisonSip, poisonImmune,
	...
	public final int[] BOWS = { 16887, 17295, 15241, 18357, 9185, 839, 845,
			847, 851, 855, 859, 841, 843, 849, 853, 857, 861, 4212, 4214, 4215,
			11235, 15701, 15702, 15703, 15704, 4216, 4217, 4218, 4219, 4220,
			4221, 4222, 4223, 6724, 4734, 4934, 4935, 4936, 4937 };
	public final int[] ARROWS = { 15243, 882, 884, 886, 888, 890, 892, 4740,
			11212, 9140, 9141, 4142, 9143, 9144, 9240, 9241, 9242, 9243, 9244,
			9245 };
	public final int[] NO_ARROW_DROP = { 15241, 4212, 4214, 4215, 4216, 4217,
	...
	public static final int[][] MAGIC_SPELLS = {
			...
			{ 12987, 52, 1978, 0, 378, 379, 14, 31, 560, 2, 562, 2, 566, 1,
					556, 1 }, // shadow rush
			{ 12901, 56, 1978, 0, 372, 373, 15, 33, 560, 2, 562, 2, 565, 1, 0,
					0 }, // blood
			// rush
	...
	public boolean fullVoidRange() {
		return playerEquipment[playerHat] == 11664
				...
				|| playerEquipment[playerLegs] == 19790
				|| playerEquipment[playerLegs] == 19787
				&& playerEquipment[playerChest] == 8839
				|| playerEquipment[playerChest] == 19785
				|| playerEquipment[playerChest] == 19787
				|| playerEquipment[playerChest] == 19789
	...
	public int calculateMaxLifePoints() {
		...
		int torvaLegs = 19002;// Torva Legs id
		int torvaBody = 19001;// Torva Body id
		int torvaHelm = 19000;// Torva Helm id
		int pernixLegs = 19005;// Pernix Chaps id
		int pernixBody = 19004;// Pernix Body id
		int pernixHelm = 19003;// Pernix Cowl id
		int virtusLegs = 19008;// Virtus Robe bottom id
		int virtusBody = 19007;// Virtus Robe top id
		int virtusHelm = 19006;// Virtus Mask id
	...
	public final int[] PRAYER_DRAIN_RATE = { 20000, 20000, 20000, 20000, 20000,
	...
	public final int ACCURATE = 0, RAPID = 1, AGGRESSIVE = 1, LONGRANGE = 2,
			BLOCK = 2, DEFENSIVE = 2, CONTROLLED = 3;
	...
	public boolean doubleHit, usingSpecial, npcDroppingItems, usingRangeWeapon,
			usingBow, usingArrows, usingOtherRangeWeapons, usingCross,
	...
	public boolean inWild() {
		if (absX > 2941 && absX < 3392 && absY > 3519 && absY < 3966
	...
	public boolean inSW() {
		return absX >= 2538 && absX <= 2590 && absY >= 4932 && absY <= 4987;
	}

	public boolean inFunPK() {
		return absX >= 2700 && absX <= 2734 && absY >= 9804 && absY <= 9832;
	}
	...
	public Player(int _playerId) {
		...
		for (int i = 0; i < this.playerItems.length; i++) {
			this.playerItems[i] = 0;
		}
		for (int i = 0; i < this.playerItemsN.length; i++) {
			this.playerItemsN[i] = 0;
		}

		for (int i = 0; i < this.playerLevel.length; i++) {
			if (i == 3) {
				this.playerLevel[i] = 10;
			} else {
				this.playerLevel[i] = 1;
			}
		}

		for (int i = 0; i < this.playerXP.length; i++) {
			if (i == 3) {
				this.playerXP[i] = 1300;
			} else {
				this.playerXP[i] = 0;
			}
		}
		...
		this.playerAppearance[0] = 0; // gender
		this.playerAppearance[1] = 7; // head
		this.playerAppearance[2] = 25;// Torso
		this.playerAppearance[3] = 29; // arms
		this.playerAppearance[4] = 35; // hands
		this.playerAppearance[5] = 39; // legs
		this.playerAppearance[6] = 44; // feet
		this.playerAppearance[7] = 14; // beard
		this.playerAppearance[8] = 7; // hair colour
		this.playerAppearance[9] = 8; // torso colour
		this.playerAppearance[10] = 9; // legs colour
		this.playerAppearance[11] = 5; // feet colour
		this.playerAppearance[12] = 0; // skin colour

		this.apset = 0;
		this.actionID = 0;

		this.playerEquipment[playerHat] = -1;
		this.playerEquipment[playerCape] = -1;
		this.playerEquipment[playerAmulet] = -1;
		this.playerEquipment[playerChest] = -1;
		this.playerEquipment[playerShield] = -1;
		this.playerEquipment[playerLegs] = -1;
		this.playerEquipment[playerHands] = -1;
		this.playerEquipment[playerFeet] = -1;
		this.playerEquipment[playerRing] = -1;
		this.playerEquipment[playerArrows] = -1;
		this.playerEquipment[playerWeapon] = -1;

		this.heightLevel = 0;

		this.teleportToX = Config.START_LOCATION_X;
		this.teleportToY = Config.START_LOCATION_Y;

		this.absX = this.absY = -1;
		this.mapRegionX = this.mapRegionY = -1;
	...
	public boolean withinDistance(int absX, int getY, int getHeightLevel) {
		if (getHeightLevel() != getHeightLevel)
			return false;
	...
	public void addToWalkingQueue(int x, int y) {
		int next = (this.wQueueWritePtr + 1) % 50;
		if (next != this.wQueueWritePtr) {
			this.walkingQueueX[this.wQueueWritePtr] = x;
			this.walkingQueueY[this.wQueueWritePtr] = y;
			this.wQueueWritePtr = next;
		}
	}
	...
	public int getNextWalkingDirection() {
		...
		// if(!Server.npcHandler.brawlerBlock(this, absX +
		// Misc.directionDeltaX[dir], absY + Misc.directionDeltaY[dir])) {
		// if (VirtualWorld.I(heightLevel, currentX, currentY, absX +
		// Misc.directionDeltaX[dir], absY + Misc.directionDeltaY[dir], 0)) {
	...
	public void updateVisiblePlayers() {
		for (int i = 0; i < Config.MAX_PLAYERS; i++) {
			...
			if (!withinDistance(PlayerHandler.players[i])
	...
	public void startAnimation(int animId) {
		if ((isDead || playerLevel[3] <= 0) && animId != 836) {
			return;
		}
	...
	protected void appendHitUpdate2(Stream str) {
		...
		str.writeByteS((poisonMask == 1) ? 2 : (hitDiff2 > 0) ? 1 : 0);
		if (poisonMask == 2)
			poisonMask = -1;
		if (playerLevel[3] <= 0) {
			playerLevel[3] = 0;
	...
	public boolean atHome() {
		return Area(3405, 3452, 3531, 3580);
	}
	...
	private ISAACRandomGen inStreamDecryption = null,
...
/**
	 * @return the bankX
	 */
	public boolean isBankX() {
		return bankX;
	}

	/**
	 * @param bankX the bankX to set
	 */
	public void setBankX(boolean bankX) {
		this.bankX = bankX;
	}
