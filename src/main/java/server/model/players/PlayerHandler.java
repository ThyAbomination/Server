...
public class PlayerHandler {
	...
	public void updatePlayer(Player plr, Stream str) {
		...
		if (plr.didTeleport) {
			plr.updateVisiblePlayers(); // so if we teleport and we are in
										// our original region we are added
										// back to the list for all the
		...
		for (int i = 0; i < addSize; i++) {
			...
			if (players[id] == null || !players[id].isActive
					|| players[id] == plr)
				continue;

			if (!plr.withinDistance(players[id])
