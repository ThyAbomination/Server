...
public class PotionMixing {
	...
	public void mixPotion2(int id, int id2) {
		try {
			...
			if (id11.equalsIgnoreCase("potion flask")
					|| id22.equalsIgnoreCase("potion flask"))
				return;
	...
	private int[][] potMixing = { { 117, 117, 113, 227 },
			...
			{ 183, 183, 2450, 227 }, { 189, 193, 2450, 227 },
			{ 193, 189, 2450, 227 }, { 3046, 3042, 3040, 227 },
			{ 2456, 2456, 2452, 227 }, { 2454, 2458, 2452, 227 },
			{ 2458, 2454, 2452, 227 } };

	// use id, use id2, new id1, new id2
	public void mixPotion(int id, int id2) {
		for (int[] aPotMixing : potMixing) {
			if ((aPotMixing[0] == id && aPotMixing[1] == id)
					|| (aPotMixing[1] == id && aPotMixing[0] == id2)) {
				c.getItems().deleteItem(id, c.getItems().getItemSlot(id), 1);
				c.getItems().deleteItem(id2, c.getItems().getItemSlot(id2), 1);
				c.getItems().addItem(aPotMixing[2], 1);
				c.getItems().addItem(aPotMixing[3], 1);
				break;
			}
		}
	}
