...
public class MagicRequirements extends MagicData {
	...
	public static boolean wearingStaff(Client c, int runeId) {
		...
		int book = c.playerEquipment[Player.playerShield];
		switch (runeId) {
		case 554:
			if (wep == 1387)
				return true;
	...
	public static boolean checkMagicReqs(Client c, int spell) {
		if (c.usingMagic && Config.RUNES_REQUIRED) { // check for runes
			if ((!c.getItems().playerHasItem(Player.MAGIC_SPELLS[spell][8],
					Player.MAGIC_SPELLS[spell][9]) && !wearingStaff(c,
					Player.MAGIC_SPELLS[spell][8]))
					|| (!c.getItems().playerHasItem(
							Player.MAGIC_SPELLS[spell][10],
							Player.MAGIC_SPELLS[spell][11]) && !wearingStaff(c,
							Player.MAGIC_SPELLS[spell][10]))
					|| (!c.getItems().playerHasItem(
							Player.MAGIC_SPELLS[spell][12],
							Player.MAGIC_SPELLS[spell][13]) && !wearingStaff(c,
							Player.MAGIC_SPELLS[spell][12]))
					|| (!c.getItems().playerHasItem(
							Player.MAGIC_SPELLS[spell][14],
							Player.MAGIC_SPELLS[spell][15]) && !wearingStaff(c,
							Player.MAGIC_SPELLS[spell][14]))) {
				c.sendMessage("You don't have the required runes to cast this spell.");
				return false;
			}
