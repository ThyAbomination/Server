...
public class AttackPlayer {
	...
	public static void applyPlayerMeleeDamage(Client c, int i, int damageMask,
		...
		if (o.prayerActive[16] || o.curseActive[7] && c.specEffect == 8
		...
		if (o.prayerActive[18] || o.curseActive[9]
				...
				&& !veracsEffect && c.specEffect != 8) { // if prayer active
															// reduce damage by
															// 40%
			damage = damage * 60 / 100;
	...
	public static void playerDelayedHit(final Client c, final int i) {
		if (PlayerHandler.players[i] != null) {
			...
			if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (o.prayerActive[17] || o.curseActive[8]
				...
				if (c.usingMagic) { // magic hit delay
					...
					if (Player.MAGIC_SPELLS[c.spellId][4] > 0) {
						c.getPA().createPlayersProjectile(pX, pY, offX, offY,
								50, 78, Player.MAGIC_SPELLS[c.spellId][4],
					...
					c.spellId = 0;
					Client o = (Client) PlayerHandler.players[i];
					if (Player.MAGIC_SPELLS[c.oldSpellId][0] == 12891
							&& o.isMoving) {
					 ...
					 * .getCombat().mageAtk())) { c.magicFailed = true; } else {
					 * c.magicFailed = false; }
					 */
					if (Misc.random(c.getCombat().mageAtk()) > Misc.random(o
							.getCombat().mageDef())) {
						c.magicFailed = false;
					} else if (Misc.random(c.getCombat().mageAtk()) < Misc
							.random(o.getCombat().mageDef())) {
						c.magicFailed = true;
					}
